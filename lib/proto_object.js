...
define(["./serializer", "brace_prototype"], function(serializer, proto) {
	...
	return p.extend({
		...
		_log_level: [-Infinity, Infinity], // The default log_level option will initially be altered by this getter until the parent level prototype chain
		toStyleString: function() {
		  ...
		  // it only returns an Object property.
			...
			return (this.style_map[this.platform].theme[this.theme+"_"+this.level].open_with || "") + formated + 
						(this.style_map[this.platform].theme[this.theme+"_"+this.level].close_with || "")
		   ...
		   // The is_chained property tells any printing commands to create a new instance or use the existing.
			...
			var parse_args = (function(args) {
				...
				for ( var x = 0; x < args.length; x++ ) 
					if ( args[x] instanceof this.parent ) {
						for ( var o in args[x].list() ) // Note: .list() is a cheap function so it can be called many times.
							this[o] = args[x][o] 
						this._log_level = args[x]._log_level
					// It is also acceptable to pass arguments inside of objects to the option member.
					  ...
					  parse_args(args[x])
						...
						this.log_title = args[x]
					} else {
						for ( var o in args[x] ) 
							if ( o in this.list() || o === "_log_level" ) 
								this[o] = args[x][o]
							else 
								return new this.parent({theme: this.theme+"_internal", level: this.internal_level, title: "Bracket Print Error"})
									.sp("Option", o, "is not a brackit print option. Use .list() to see all user configurable options:").line(this.list()).log()
					}
			}).bind(print)
		...
		get log() {
			...
			return (function() {
				...
				// Append the logging title if one is set.
				var theme = this.theme+"_"+this.level, title = "", formated_title, uses_apply
				if ( this.title && this.title_stamp )
					title = "[" + ( typeof this.title_stamp === "function" && this.title_stamp() || String(this.title_stamp) ) + "] "

				if ( this.style) {
 					// Turn the entire output string into an array to pass with the console.log.apply

					uses_apply = this.style_map[this.platform].format.length >= 3
					formated_title = this.style_map[this.platform].format("title", title, uses_apply && this.apply_arguments || null)
					console.log.apply(console, this.apply_arguments.concat([this.toStyleString(formated_title)]))
				} else {
