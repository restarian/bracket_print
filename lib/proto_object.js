...
define(["./serializer", "brace_prototype"], function(serializer, brace_prototype) {
	...
	brace_proto.add_hidden_qualifier("_internal_error")	
	return brace_proto.extend({
		...
		get _chain() {
			 ...
			 return function() {
				...
				var platform, theme  
				// In case a bad option value was set in the call chain.
				if ( !(platform = this.current_platform) || !(theme = this.current_theme) )
					return this 

				var indentation = this.indentation_string.replace(/\t/g, platform.denote_tab||"\t")
										.replace(/\n/g, platform.denote_line||"\n").replace(/\ /g, platform.denote_space||" ")
			...
			return (function() {
				...
				if ( this.style) {
					...
					if ( title ) {
						...
						if ( ! (style_value = ("title" in theme && theme["title"] || theme.base))  ) 
							return new this.parent(this, {level: this.internal_level||this.level, log_title: "Bracket Print Error"})
								.s("There is not a style value set for").a(".", theme, ".", style_value).s("or a")
								.a(platform, ".", theme, ".base value.").log_true() && this
						else {
							formated_title = platform.format(style_value, title, platform.format.length >= 3 && tmp_apply_args || undefined)
