...
define(["./serializer", "brace_option"], function(serializer, brace_option) {
	...
	return option.extend({
		...
		// fast (especially without arguments), as it only returns an instance property.
			...
			return wrap_start + (platform.wrap_start||"") + formated + wrap_end + (platform.wrap_end||"")
			...
			return (function() {
				...
				if ( this.style ) {
					...
					if ( this.title ) 
						...
						formated_title = platform.format(theme["title"] || theme.base || "", this._title(), platform.format.length >= 3 && tmp_apply_args || undefined)

					output = ([formated_title + this.toStyleString()]).concat(tmp_apply_args.concat(this._apply_arguments))
				}
				else
					output = [this._title() + this.plain]
