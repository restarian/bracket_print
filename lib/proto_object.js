			...
			if ( !val )
			 ...
			 for ( var x = 0; x < range.length; x+=2 ) {
				...
				if ( range[x] !== range[x] || range[x+1] !== range[x+1] ) {
					...
					if ( !this._is_error )
						new this.parent({_is_error: true}, this, 
							{level: parseInt(this.internal_level) === parseInt(this.internal_level) ? parseInt(this.internal_level) : this.level, log_title: "Bracket Print Error"})
			...
			var parse_args = (function(args) {
				...
				for ( var x = 0; x < args.length; x++ ) {
					...
					if ( Object.prototype.toString.call(args[x]) === "[object Arguments]" ) {
						...
						if ( typeof opt === "string" ) {
							...
							for ( var n in opt ) {
								...
								if ( n in this.list() ) {
									...
									new this.parent({_is_error: true}, this, {level: this.internal_level||this.level, log_title: "Bracket Print Error"})
						...
						return new this.parent({_is_error: true}, this, {level: this.internal_level||this.level, log_title: "Bracket Print Error"})
