...
Bracket Print resides under the LGPL version 3
...
define(["./proto_object", "./style_map"], function(proto_object, style_map) {
	...
	Print.prototype.__defineGetter__("currentPlatform", function() {
		...
		if ( typeof this.style_map[this.platform] !== "object" || this.style_map[this.platform] === null  ) {
			if ( !this._is_error ) 
				new this.parent({_is_error: true}, this, {style: false, platform: "none", log_title: "Bracket Print Error", character_limit: 300,
	...
	Print.prototype.__defineGetter__("currentTheme", function() {
	...
	// theme option is not found in the style platform. 
		...
		if ( platform.import_theme_from ) {
			if ( !platform.import_theme_from in this.style_map ) {
				if ( !this._is_error )
					new this.parent({_is_error: true}, this, {style: false, log_title: "Bracket Print Error", character_limit: 300,
						level: parseInt(this.internal_level) === parseInt(this.internal_level) ? parseInt(this.internal_level) : this.level})
	...
	// The append_string member will return true unless it reaches a character_limit. This is the way which bracket print compounds strings together.
		...
		if ( this.style ) {
			...
			var theme_name = this.theme+"_"+this.level, style_value
			if ( !(style_value = ((("theme_override" in platform) && (theme_name in platform.theme_override) && platform.theme_override[theme_name][style_name]) || (theme[style_name] || theme.base)) ))
				return new this.parent(this, {style: false, log_title: "Bracket Print Error", character_limit: 300,
					level: parseInt(this.internal_level) === parseInt(this.internal_level) ? parseInt(this.internal_level) : this.level})
				.log_true("There is not a style value set for", style_name, "with theme", this.theme+"_"+this.level, "in platform", this.platform)
			...
			if ( this.formated.length ) {
				...
				var wrap = ("theme_override" in platform) && platform.theme_override[this.theme+"_"+this.level]
				if ( wrap === null || typeof wrap !== "object" ) {
					wrap_start = theme.wrap_start||""
					wrap_end = theme.wrap_end||""
				}
				else {
					if ( "wrap_start" in wrap )
						wrap_start = wrap.wrap_start||""
					if ( "wrap_end" in wrap )
						wrap_end = wrap.wrap_end||""
				}

				this.formated += wrap_end + (platform.wrap_end||"") + plain + wrap_start + (platform.wrap_start||"")
			}
			else 
				this.formated += plain
		}
