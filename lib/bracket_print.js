...
define(["require", "./proto_object", "./style_map"], function(require, obj, style_map) {
	...
	obj.append_string = function(style_name, str) {
		...
		if ( this.plain.length < this.character_limit ) {
			...
			this.plain += plain
			var theme, platform = this.style_map[this.platform].import_theme || this.platform
			...
			if ( !theme )
				return new this.parent(this, {level: this.internal_level||this.level, log_title: "Bracket Print Error"})
					.log_true("The requested theme", theme, "is not included.")
			...
			if ( this.style ) {
				...
				var style_value = style_name in this.style_map[platform].theme[theme] && this.style_map[platform].theme[theme][style_name] || 
											this.style_map[platform].theme[theme].base || undefined

				if ( !style_value ) 
					// This only happens when the style theme Object contains no properties.
					return new this.parent(this, {level: this.internal_level||this.level, log_title: "Bracket Print Error"})
						.s("There is not a style value set for").a(platform, ".", theme, ".", style_value).s("or a")
						.a(platform, ".", theme, ".base value.").log_true()

				this.formated += this.style_map[platform].format(style_value, plain, uses_apply && this.apply_arguments || undefined)
			} else {

				if ( this.formated.length ) {
					var platform = this.style_map[this.platform]
					var style = platform.theme[this.theme+"_"+this.level]
					this.formated += (style && style.close_with || platform && platform.close_with || "") + plain +
											(style && style.open_with || platform && platform.open_with || "")
				}
				else 
					this.formated += plain
			}
