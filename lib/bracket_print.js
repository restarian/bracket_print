...
define(["require", "./proto_object", "./style_map"], function(require, obj, style_map) {
	...
	obj.parent = function() { 
		return Print.apply(Print, arguments)
	}
	Print.prototype = obj
	...
	obj.append_string = function(style_name, str) {
		...
		if ( this.use_style ) {
			...
			// If style index has an odd length
			if ( this.style_index.length & 1 )
				this.style_index.push({plain: this.plain_index[this.plain_index.length-1], formated: this.formated.length})	

			var theme = "", uses_apply = this.style_map[this.platform].format.length >= 3,
				platform = this.style_map[this.platform].use_theme_from || this.platform
			...
			else if ( this.style_map[platform].theme[this.style_map[platform].default_theme] )
