...
Print.prototype.Shim.prototype = {
  ...
  get pass() {
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg && (msg instanceof Print.prototype.Shim) ) {
     ...
     else if ( typeof Buffer !== "undefined" && msg.__proto__.constructor === Buffer ) {
       return this._stringify_object(msg.toString())
     }
     else if ( msg === null ) {
       ...
       this._stack_size++
       if ( this._stack_size > 500 ) {
         this._stack_pause_obj = msg
         this._stack_size = 0
//         return ""
       }
       var m = "", value_keys_length = Object.keys(msg).length
       var proto_keys_length = 0
       ...
       if ( typeof msg === "function" ) { //Object.prototype.toString.call(msg) === "[object Function]" ) {
        ...
        // }
         ...
         var b_text = is_array&&"["||"{" + (this.compress_level < 3 && this.denote_space || "")
         if ( !value_keys_length ) {
           if ( proto_keys_length ) {
             b_text += is_array&&"]"||"}"+modify_string.blue(".__proto__ = ")+"{" +compress_sep+compound+indent_string
           } else {
             b_text += is_array&&"],"||"}," + object_separator
           }
         }
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         //if ( cnt === 1 || (proto_keys_length && cnt-value_keys_length+2 === proto_keys_length) || typeof msg[o] === "object" ) {

         if ( cnt === 1 || typeof msg[o] === "object" ) {
           str_add += compress_sep+compound+indent_string
         }
         if ( !is_array ) {
           // _stringify_object is used here to format the qualifier strings.
         ...
         // recursively call this function to parse the object. The indent_string is added to the begining to nest the output
              var value = this._stringify_object(msg[o], indent_string, compound+indent_string, ++level)
         //m += value + modify_string.black(",") + object_separator
         str_add += value + "," + object_separator
         ...
         if ( cnt === value_keys_length || cnt === value_keys_length+proto_keys_length ) {
           ...
           str_add = str_add.substr(0, str_add.length-1-object_separator.length)+compress_sep+compound
           ...
           if ( value_keys_length && proto_keys_length && cnt === value_keys_length )
              str_add += is_array&&"]"||"}"+modify_string.grey(".__proto__ = ")+"{" +compress_sep+compound+indent_string
           else
              str_add += is_array&&"],"||"}," + object_separator
         }
       ...
       str_add = str_add.substr(0, str_add.length-object_separator.length) // Remove last denote and comma at the end of the Object loop
       if ( compound )
          str_add += compress_sep+compound+m
