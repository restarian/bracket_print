...
Logger.prototype = {
  ...
  new_copy: function() {
    ...
    // new_copy sinply calls the constructor with a Logger instance so this just forwards the arguemtns as well.
...
Logger.prototype.Print_logger.prototype = {
  ...
  new_copy: function() {
    ...
    // The options are set from this copy unless another Print was passed in. This can be done at the Logger Constructor as well.
  ...
  mutable_options: function() {
    ...
    return {
      ...
      platform: typeof this.platform !== "undefined" ? this.platform : (Logger.is_node_run && "nodejs" || "browser"),
        ...
        for ( var n = 0; n < level-1; n++ ) {
          for ( var x = 0; x < this._cache[n].length; x++ ) {
            if ( msg === "object" && msg === this._cache[n][x] ) {
               return "!! Circular duplicate of ->\n"+Logger(this).sp(this._cache[n][x]).text() // Circular reference found, discard object processing
            }
          }
        }
...
//  console.log(n, typeof Logger.prototype[n] === "undefined")

  if ( typeof Logger.prototype[n] === "undefined" && n.charAt(0) !== "_" ) {
