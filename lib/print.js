...
// This prototype is created two levels deep (__proto__.__proto__), into the Print() prototype chain. Any memeber names with a underscore prepended will not have a
...
Print.prototype = {
  ...
  _mutable_options: function() {
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     var modify_string = this._colors
     modify_string.theme = this.theme
     modify_string.line_denote = this.line_denote
     modify_string.max_line_characters = this.max_line_characters

     if ( msg instanceof Print ) {
       return this._stringify_object(msg.toString(), this.denote_line+compound, this.max_line_characters)
     }
     else if ( msg === null ) {
       return modify_string.null("null")
     }
     else if ( typeof msg === "boolean" ) {
       return modify_string.boolean(msg.toString(), this.denote_line+compound, this.max_line_characters)
     }
     else if ( msg !== msg ) { // NaN is the only js object which is not equal to itself
       return modify_string.not_a_number("NaN", this.denote_line+compound, this.max_line_characters)
     }
     else if ( msg instanceof Error ) {
       var stack_string = msg.stack.split("\n")
       modify_string.namespace(stack_string.splice(0,1) + this.denote_line, this.denote_line+compound, this.max_line_characters)
       return modify_string.function_body(stack_string.join(this.denote_line), this.denote_line+compound, this.max_line_characters)
     }
     else if ( typeof msg === "undefined" ) {
       return modify_string.undefined("undefined", this.denote_line+compound, this.max_line_characters)
     }
       ...
       for ( var n = 0; n < level; n++ ) {
         for ( var x = 0; x < this._cache[n].length; x++ ) {
           if ( typeof msg === "object" && msg === this._cache[n][x] ) {
             ...
             modify_string.scope_container("[[ ", this.denote_line+compound, this.max_line_characters)
             modify_string.function_body(circ_text, this.denote_line+compound, this.max_line_characters)
             modify_string.scope_container("{ ", this.denote_line+compound, this.max_line_characters)
             modify_string.function_body(obj_qualifier, this.denote_line+compound, this.max_line_characters)
             return modify_string.scope_container(" } ]]", this.denote_line+compound, this.max_line_characters)
           }
         }
       }
       ...
       if ( typeof msg === "function" ) {
         if ( (/\[native code\]/i).test(msg.toString()) ) {
            modify_string.function_constructor(msg.toString().replace(/(.*)\n*/g, "$1"), this.denote_line+compound, this.max_line_characters)
         }
         else if ( this.compress_function ) {
           modify_string.function_body( msg.toString().match(/(function[^\{]+).*/i).pop(), this.denote_line+compound, this.max_line_characters)
           modify_string.scope_container("{", this.denote_line+compound, this.max_line_characters)
           modify_string.comma("...", this.denote_line+compound, this.max_line_characters)
           modify_string.scope_container("}", this.denote_line+compound, this.max_line_characters)
         }
         else {
            //var line = this.denote_line
            //var str = modify_string.function_body(msg.toString().replace(/(.*)\}[\ ,\n]*/g, "$1"), this.denote_line+compound, this.max_line_characters)

          //  str.formated = str.formated.replace(/\)\ *\{/, ") " + modify_string.scope_container("{").formated + modify_string.double_quote(null).formated)
          //  .replace(/\n/g, function(match, index, base) { return modify_string.indent_string(line + compound).formated })

          //      + modify_string.indent_string(this.denote_line + compound) + modify_string.scope_container("}")

          //   formated += modify_string.formated
          //   plain += modify_string.plain
         }
         formated += modify_string.formated
         plain += modify_string.plain
       }
       else {
         modify_string.scope_container(is_array&&"["||"{" + (this.compress_level < 3 && this.denote_space || ""), this.denote_line+compound, this.max_line_characters)
         if ( !value_keys_length ) {
           if ( proto_keys_length ) {
              modify_string.scope_container(is_array&&"]"||"}", this.denote_line+compound, this.max_line_characters)
              modify_string.namespace(".__proto__ = ", this.denote_line+compound, this.max_line_characters)
              modify_string.scope_container("{"+ compress_sep, this.denote_line+compound, this.max_line_characters)
              modify_string.indent_string(compound + indent_string, this.denote_line+compound, this.max_line_characters)
              // If the __proto__ object is nesed than this will correctly navagate into it.
             var next_level = this._stringify_object(msg.__proto__, indent_string, compound + indent_string, level+1)
           } else {
             modify_string.scope_container(is_array&&"]"||"}")
             modify_string.comma("," + object_separator)
           }
         }
         formated += modify_string.formated
         plain += modify_string.plain
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( cnt === 1 || is_literal_object || this.compress_level === 1 ) {
           modify_string.indent(compress_sep + compound + indent_string, this.denote_line+compound, this.max_line_characters)
           if ( cnt === 1 && msg.valueOf() !== msg ) {
             modify_string.namespace("[[PrimitiveValue]]", this.denote_line+compound, this.max_line_characters)
             modify_string.colon(":" + object_separator, this.denote_line+compound, this.max_line_characters)
             var more = this._stringify_object(msg.valueOf(), indent_string. compound + indent_string, level+1)
             modify_string.comma("," + compress_sep, this.denote_line+compound, this.max_line_characters)
             modify_string.indent(indent_string, this.denote_line+compound, this.max_line_characters)
             formated += modify_string.formated
             plain += modify_string.plain
           }
         ...
         if ( !is_array ) {
           ...
           var more = this._stringify_object(o)
           modify_string.colon(":" + object_separator, this.denote_line+compound, this.max_line_characters)
           formated += modify_string.formated
           plain += modify_string.plain
         }
         ...
         if ( value ) {
           formated += value.formated
           plain += value.plain
         }
         modify_string.comma("," + object_separator, this.denote_line+compound, this.max_line_characters)
         ...
         if ( cnt === value_keys_length || cnt === value_keys_length+proto_keys_length ) {
           ...
           var comma = modify_string.comma(",", this.denote_line+compound, this.max_line_characters, modify_string.NO_STORAGE)
           formated = formated.substr(0, formated.length - comma.formated.length - object_separator.length)
           plain = plain.substr(0, formated.length - comma.plain.length - object_separator.length)

           modify_string.indent(compress_sep + compound, this.denote_line+compound, this.max_line_characters)
           ...
           if ( value_keys_length && proto_keys_length && cnt === value_keys_length ) {
              ...
              modify_string.namespace("__proto__", this.denote_line+compound, this.max_line_characters)
              modify_string.colon(":" + object_separator, this.denote_line+compound, this.max_line_characters)
              modify_string.scope_container("{" + compress_sep + compress_sep, this.denote_line+compound, this.max_line_characters)
              var str_add = this._stringify_object(msg.__proto__, indent_string, compound + indent_string, level+1)
           }
           else {
              modify_string.scope_container(is_array&&"]"||"}", this.denote_line+compound, this.max_line_characters) +
              modify_string.comma("," + object_separator, this.denote_line+compound, this.max_line_characters)
       ...
       if ( !level )
         this._is_from_object = false
       return {formated: formated, plain: plain}
     }
     else if ( typeof msg === "number" ) {
       return modify_string.number(msg.toString(), this.denote_line+compound, this.max_line_characters)
     }
     else if ( typeof msg === "string" ) {
       // this is set if the sting is from an object
       if ( this._is_from_object && this.quoting.toLowerCase() === "double") {
         modify_string.double_quote("\"", this.denote_line+compound, this.max_line_characters)
         modify_string.string(msg, this.denote_line+compound, this.max_line_characters)
         return modify_string.double_quote("\"", this.denote_line+compound, this.max_line_characters)
       }
       else if ( this._is_from_object && this.quoting.toLowerCase() === "single") {
         modify_string.single_quote("\'", this.denote_line+compound, this.max_line_characters)
         modify_string.string(msg, this.denote_line+compound, this.max_line_characters)
         return modify_string.single_quote("\'", this.denote_line+compound, this.max_line_characters)
       }
       else {
         return modify_string.string(msg, this.denote_line+compound, this.max_line_characters)
       }
     }
     else {
       return modify_string.namespace(msg.toString(), this.denote_line+compound, this.max_line_characters)
     }
   },
}
...
var color_map = {
 ...
 terminal: Object.create({
   light: {
      single_quote: "\033[1;37m",
      double_quote: "\033[1;37m",
      number: "\033[0;32m",
      string: "\033[0;35m",
...
for ( var x in color_map )
  ...
  // to the buffers.
   ...
   color_map[x]._add_line_break = function(str, line_denote, max_line_characters) {
  ...
  // TODO make this a const with defineProperty
  color_map[x].NO_STORAGE = true
...
// This is the first level prototype Object for any Color convering plugin.
  ...
  this[val] = function(str, denote_line, max_line_characters, flag) {
    ...
    //console.log("++++++", max_line_characters, str)
...
Print.Terminal_colors = function() {
   // this will replace the terminal color output with browser complient console.log modify_string.
   this.theme = "light"
   this.use_color = true
}
...
//Print().set_option({max_depth: 3, max_line_characters: 30}).log(a)
