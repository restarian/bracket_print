...
//         This.output += This._colors.default_color_tag()
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg && (msg instanceof Print.prototype.Shim) ) {
       ...
       // TODO: The processing should stop if the max_depth or max_length limits are reached to avoid call stack overflow or exceding the maximum variable
       // data size.
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         var is_literal_object = !(typeof msg[o] !== "object" || msg[o] === null || msg[o] === undefined || msg[o] !== msg[o])
         ...
         if ( level < this.max_depth-1 ) {
            ...
            value = this._stringify_object(msg[o], indent_string, compound + indent_string)
         } else if ( is_literal_object ) {
...
//         character_count += (value+","+object_separator).length
 ...
 Print.Browser_colors.prototype = {
   ...
   format: function(args) {
     ...
     //console.log("color: "+args.callee.name)
...
;(function(o) {
  ...
  if ( cnt > 10 )
    ...
    var ss = {my_array: [true, false, 5, Function, Number, 5, Object, Error], cool: "joe", bob: "real"}
    o[cnt] = {here: [1, 2, null, 4*"ss", undefined, Function, true, false, Number, Object], cool: "joe", bob: "real"}
  arguments.callee(o[cnt])
})(o)

//Print().set_option({compress_function: false, max_depth: 103, use_title: false, quoting: "single"}).log(o)
