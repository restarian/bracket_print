  ...
  _mutable_options: function() {
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg instanceof Print ) {
       ...
       return modify_string.namespace(stack_string.splice(0,1)) + this.denote_line + modify_string.function_body(stack_string.join(this.denote_line))
     }
       ...
       var output = new String(), output.plain = ""
       ...
       for ( var n = 0; n < level; n++ ) {
         for ( var x = 0; x < this._cache[n].length; x++ ) {
           if ( typeof msg === "object" && msg === this._cache[n][x] ) {
             ...
             modify_string.scope_container("[[ ") + modify_string.function_body(circ_text) + modify_string.scope_container("{ ") +
                  modify_string.function_body(obj_qualifier) + modify_string.scope_container(" } ]]")
             output += modify_string.formated // These will be cleared the next time modify_string is called.
             output.plain += modify_string.plain
             return output
         }
       }
       ...
       if ( typeof msg === "function" ) {
         ...
         if ( (/\[native code\]/i).test(msg.toString()) ) {
            f_text = modify_string.function_constructor(msg.toString().replace(/(.*)\n*/g, "$1"))//+compound
         }
         else if ( this.compress_function ) {
           f_text = msg.toString().match(/(function[^\{]+).*/i).pop()
           f_text = modify_string.function_body(f_text)+modify_string.scope_container("{")+modify_string.comma("...")+modify_string.scope_container("}")
         }
         else {
            var line = this.denote_line
            f_text = modify_string.function_body(msg.toString().replace(/(.*)\}[\ ,\n]*/g, "$1"))
            .replace(/\)\ *\{/, ") "+modify_string.scope_container("{")+modify_string.double_quote(null))
            .replace(/\n/g, function(match, index, base) { return modify_string.indent_string(line + compound) })
                + modify_string.indent_string(this.denote_line + compound) + modify_string.scope_container("}")
         }
         m += this._add_line_break(this.denote_line+compound, f_text)
         //m += f_text
       }
       else {
         var b_text = modify_string.scope_container(is_array&&"["||"{") + (this.compress_level < 3 && this.denote_space || "")
         if ( !value_keys_length ) {
           if ( proto_keys_length ) {
              b_text += modify_string.scope_container(is_array&&"]"||"}") + modify_string.namespace(".__proto__ = ") + modify_string.scope_container("{") + compress_sep +
                  modify_string.indent_string(compound + indent_string)
              ...
              b_text += modify_string.scope_container(is_array&&"]"||"}") + modify_string.comma(",") + object_separator
           }
         }
         m += this._add_line_break(this.denote_line+compound, b_text)
         //m += b_text
       }
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( cnt === 1 || is_literal_object || this.compress_level === 1 ) {
           str_add += compress_sep + modify_string.indent(compound + indent_string)
           if ( cnt === 1 && msg.valueOf() !== msg ) {
             var more = modify_string.namespace("[[PrimitiveValue]]") + modify_string.colon(":") + object_separator
             more += this._stringify_object(msg.valueOf(), indent_string. compound + indent_string, level+1)
             more += modify_string.comma(",") + compress_sep + modify_string.indent(indent_string)
         ...
         if ( !is_array ) {
           ...
           var more = this._stringify_object(o) + modify_string.colon(":") + object_separator
           character_count += more.length
           str_add += more
         }
         ...
         str_add += value + modify_string.comma(",") + object_separator
...
//         character_count += (value+","+object_separator).length
         ...
         if ( cnt === value_keys_length || cnt === value_keys_length+proto_keys_length ) {
           ...
           str_add = str_add.substr(0, str_add.length-modify_string.comma(",").length-object_separator.length)
           if ( modify_string.use_color && modify_string instanceof Print.Browser_colors ) {
              modify_string.css_arguments.splice(-2)
           }
           str_add += compress_sep + modify_string.indent(compound)
           ...
           if ( value_keys_length && proto_keys_length && cnt === value_keys_length ) {
              str_add += modify_string.scope_container(is_array&&"]"||"}") + modify_string.namespace("__proto__") + modify_string.colon(":") +
                  object_separator + modify_string.scope_container("{") + compress_sep + compress_sep
          ...
          //    str_add += indent_string + modify_string.red("__proto__") + modify_string.blue(":") + modify_string.cyan("{") + compress_sep + compound
              ...
              str_add += modify_string.scope_container(is_array&&"]"||"}") + modify_string.comma(",") + object_separator
           }
         }
...
// e.g. .__proto__ [[primitivevalue}}1
       m += str_add
       m = m.substr(0, m.length-modify_string.comma(",").length-object_separator.length)
       if ( modify_string.use_color && modify_string instanceof Print.Browser_colors ) {
         modify_string.css_arguments.splice(-2)
       }
       if ( !level )
       ...
       if ( this._is_from_object && this.quoting.toLowerCase() === "double") {
         return modify_string.double_quote("\"")+modify_string.string(msg)+modify_string.single_quote("\"")
       }
       else if ( this._is_from_object && this.quoting.toLowerCase() === "single") {
         return modify_string.single_quote("\'")+modify_string.string(msg)+modify_string.single_quote("\'")
       }
       else {
...
// This is the first level prototype Object for any Color convering plugin.
  ...
  this[val] = function(str) {
    ...
    if ( this._retrieved ) {
      ...
      this.plain = this.formated = "" // Clear the buffers if they were accessed before this call
    }
    this.plain += str.toString()
    if ( !this.use_color )
      return this.plain

    // Call the format function which are specified in the first level prototype of the constructor
    return this.formated += this.format(str, this.__proto__[this.theme][arguments.callee.command_name])
  }
...
Print().set_option({max_depth: 3, max_line_characters: 30, theme: "dark"}).log(a).set_option({theme: "light"}).log()
