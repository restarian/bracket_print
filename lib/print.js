...
var Print = function() {
  // Accepts all of the parameters as set_option does.
  ...
  var copy_setting = function(setting) {
    // Copy the settings Object passed in to the prototype of the current instance. This effectively copies all of the nessesary options to the current
    // instance from the previous one.

    // Object.keys will not include properlies in the prototyp so that only settings which are set as instance data are copied.
  ...
  // void is used the same as the parenthisis enclosure method to make functions callable in the declaration. It returns the function declaration to void which returns
  // it to the command line. Without void something else in the expression evaluation of the function is returned to the command line (which returns undefined).
  void function parse_args(args) {
    ...
    for ( var x = 0; x < args.length; x++ ) {
      if ( args[x] instanceof Print ) {
        copy_setting.call(this, args[x]._mutable_options)
...
Print.prototype = {
  ...
  // This is used to set the level of any messages which are logged internally in the Print library.
  internal_level: 2,
}

// The simple list of user configurable options which is more convenient to make at library load time. This will need to be set again if options are added in run-time.
Print.prototype.mutable_options = Object.keys(Print.prototype)

Print._descoped_use_level_setter = function(value) {
  ...
  value.toString().replace(/\ +/g, "").replace(/\-\-([^\-]{1})/g, "-minus$1").replace(/^\-/, "minus").split(",").forEach(function(val) {
    ...
    for ( var x = 0; x < range.length; x+=2 ) {
      ...
      if ( range[x] !== range[x] || range[x+1] !== range[x+1] )
        return Print({level: this.internal_level}).log("The value set to use_level can not be parsed as an integer:", range)
...
Print.prototype._use_level = [-Infinity, Infinity], // The default use_level option will initially be altered by this getter until the parent level prototype chain
...
Print.prototype.__defineSetter__("use_level", function(value) { Print._descoped_use_level_setter.call(this, value) })
Print.prototype.__defineGetter__("use_level", function() {
...
Print.prototype.__defineGetter__("_mutable_options", function() {
  ...
  // then populated with the highest priority value (top most in the prototype chain). It is nessesary to gather all the mutable_options at the time they
  // are needed sense the prototype chain is expected to change as the options are set in different ways.
  var opt = {}
...
Print.prototype.toColorString = function() {
  ...
  return this._print_command(this._last_command || "space").apply(this, arguments).bufferize.formated + (this.bufferize.apply_arguments &&
      this.bufferize.apply_arguments.length && this.bufferize.apply_arguments.join(" ") || "")
}
Print.prototype.toString = function() {
  ...
  return this._print_command(this._last_command || "space").apply(this, arguments).bufferize.plain
}
Print.prototype.set_option = function() {
  // The other way of setting the options which resturns this to maintain call chains. Objects, Print() instances and Strings can be passed into
  ...
  var p = ((this._is_chained || this._from_constructor) && this) || Print(this)
  ...
  for ( var i = 0; i < arguments.length; i++ ) {
    ...
    // TODO: this is a tad ineficient
    var mutable = p._mutable_options
...
Print.prototype.__defineGetter__("log", function() {
 ...
 if ( !in_range )
   ...
   if ( this.bufferize.formated ) {
     ...
     if ( Object.prototype.toString.call(this.bufferize.apply_arguments).toLowerCase() === "[object array]" )
       c_args = c_args.concat(color_copy.apply_arguments).concat(this.bufferize.apply_arguments)
...
Print.prototype._serialize = function(msg, indent_string, previous_indent, level) {
   ...
   this._serialize(msg.toString())
    ...
    for ( var x = 0; x < keys.length; x++ ) {
      ...
      if ( level < this.depth_limit-1 || !is_property_based ) {
        ...
        this.bufferize.namespace(Print().add("[[", is_array && "Array" || "Object", " with ", len, " propert")
        .add(len === 1 && "y" || "ies", "]]")
        )
      }
...
Print._remove_call = function() {
  ...
  if ( Object.prototype.toString.call(this.apply_arguments).toLowerCase() === "[object array]" )
...
Print.terminal = function() {
   ...
   this.denote_space = " "
   this.denote_add = ""
   this.format = function(val, color_value) {
...
Print.browser = function() {
 // this will replace the terminal color output with browser complient console.log this.bufferize.
  ...
  this.format = function(val, color_value) {
    ...
    this.apply_arguments.push(color_value)
    return "%c"+val
  }
}
...
var a  = Print({use_title: false, compress_level: 4})
a.log({__proto__:{__proto__:{v:1}}})
