/*
  Wizkit Print is a logger library for the termanal and browser console. 
...
Print.prototype = {
  ...
  new_copy: function() {
    ...
    // new_copy sinply calls the constructor with a Print instance so this just forwards the arguemtns as well.
  ...
  Print_logger: function(settings) { // The Print constructer is sub-classed here in Wizkit
...
Print.prototype.Print_logger.prototype = {
  ...
  new_copy: function() {
    ...
    // The options are set from this copy unless another Print was passed in. This can be done at the Print Constructor as well.
    for ( var x = 0; x < arguments.length; x++ )
      if ( arguments[x] instanceof Print.prototype.Print_logger ) {
...
//        var title_str = This.log_title ? this.colors.blue("[")+This.log_title+this.colors.blue("] ") : ""
        ...
        if ( This.colors instanceof Print.Browser_colors )
  ...
  _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
    ...
    if ( msg && msg instanceof Print.prototype.Print_logger ) {
      msg = this._stringify_object(msg.text())
    }
    else if ( msg === null ) {
        ...
        for ( var n = 0; n < level-1; n++ ) {
          for ( var x = 0; x < this._cache[n].length; x++ ) {
            if ( msg === "object" && msg === this._cache[n][x] ) {
               return "!! Circular duplicate of ->\n"+Print(this).sp(this._cache[n][x]).text() // Circular reference found, discard object processing
            }
          }
        }
...
var fake_logger = new (function() { // The FakePrint is used to disable logging when debug is set to false (it is efficiant this way).
    ...
    for ( var o in Print.prototype.Print_logger.prototype )
      this[o] = f
...
for ( var n in Print.prototype.Print_logger.prototype ) {
//  console.log(n, typeof Print.prototype[n] === "undefined")
