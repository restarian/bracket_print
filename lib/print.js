...
var Print = function() {
  ...
  if ( !arguments.length )
    copy_setting(this._mutable_options)
  ...
  var parse_args = (function(args) {
    ...
    for ( var x = 0; x < args.length; x++ ) {
      if ( args[x] instanceof Print ) {
        copy_setting(args[x]._mutable_options)
...
Print.prototype.__defineGetter__("clear", function() {
 ...
 Print.prototype.__defineGetter__("log", function() {
   ...
   if ( !in_range )
     ...
     if ( this.bufferize.formated ) {
       ...
       if ( this.bufferize.apply_arguments && this.bufferize.apply_arguments.length )
 ...
 Print.prototype._serialize = function(msg, indent_string, previous_indent, level) { // previous_indent will be placed at the start of each new object in the output (used internally)
   ...
   if ( msg instanceof Print ) {
     ...
     var keys = Object.keys(msg), is_primitive_object = false
     ...
     if ( !keys.length ) {
       ...
       // sense it is a property of the primitive value (not the Object). Therefore the length property is added as a design choice.
        if ( typeof msg.length !== "undefined" ) {
          ...
          this.bufferize.colon(":" + compress_separator)
          this._serialize(msg.legnth, indent_string, previous_indent + indent_string, level+1)
          this.bufferize.comma("," + compress_separator)
        }
      this.bufferize.remove_call(-1)
      }
     ...
     for ( var x = 0; x < keys.length; x++ ) {
       ...
       // Any non-literal Objects will be printed on the last depth parsing level (which is what the !is_literal_object test is for).
         if ( level < this.depth_limit-1 || !is_literal_object ) {
            this._serialize(msg[o], indent_string, previous_indent + indent_string, level+1)
         } else {
            var len = Object.keys(msg[o]).length
...
var tt = Print()
console.log(tt.bufferize)
//console.log(Print().sp("ABC").bufferize.__proto__.__proto__[Print().bufferize.theme])// + this.level])
