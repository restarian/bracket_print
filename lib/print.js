...
Print._mapping = function(str) {
  ...
  if ( this.use_color ) {
    ...
    if ( /\S/.test(plain) ) {
      ...
      this.formated += this.format(plain, this.__proto__[theme][arguments.callee.command_name])
    } else {
      this.formated += plain
    }
  }
...
for ( var x in color_map ) {
  ...
  Object.keys(color_map[x][color_map[x]._installed_theme[0]]).forEach(function(val, index) {
    // This is the first level prototype Object for any Color convering plugin.

    this[val] = Print._mapping
