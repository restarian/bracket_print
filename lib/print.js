...
Print.prototype = Object.create(
  {
    ...
    level: 1,
    // this._use_level is contained one level up in the chain and should stay that way.
//    get use_level() { return this._use_level },
    // This setter is used by both this level and the level obove (parent) setter so that the parent can be deleted without loosing the inherent
    // functionality of the use_level property (which parses as a string and re-sets its value).
//    set use_level(value) { Print._descoped_use_level_setter.call(this, value) },
...
Print._descoped_use_level_setter = function(value) {
  ...
  // more human friendly denotationss. When strings are passed in: ranges are two numbers separated with a dash and single value levels are separated
  ...
  value.toString().replace(/\ +/g, "").replace(/\-\-([^\-]{1})/g, "-minus$1").replace(/^\-/, "minus").split(",").forEach(function(val) {
    ...
    var type_a = range[0].match(/(minus)*(Infinity)*/g) || []
    var type_b = range[1].match(/(minus)*(Infinity)*/g) || []

console.log("here", range)
    if ( type_a[1] )
      range[0] = Infinity
    else
      range[0] = parseInt(range[0].replace(/minus/, ""))

console.log("here", range, type_b)
    if ( type_b[1] )
      range[1] = Infinity
    else
      range[1] = parseInt(range[1].replace(/minus/, ""))
...
Print.prototype.__defineGetter__("clear", function() {
 ...
 Print.prototype._print_command = function(call_name) {
   ...
   return (function() {
     ...
     for ( var x = 0; x < this.use_level.length; x += 2 )
       if ( this.level >= this.use_level[x] && this.level <= this.use_level[x+1] ) {
 ...
 Print.prototype.__defineGetter__("log", function() {
   ...
   return (function() {
     var in_range = false
         console.log("fdsfsdfdsf", this.use_level, this.level)
     for ( var x = 0; x < this.use_level.length; x += 2 )
...
Print.prototype.use_level = [-Infinity, Infinity]
console.log(Print.prototype.use_level)
/*
var a = Print()

//a.log("sss")
//console.log(a.use_level)
//var a = {cool: "joes", here: {}}
Print.prototype.use_level = "5 -5"
console.log(a.use_level)

a.use_level = "15 -5"
console.log(a.use_level)
console.log(Print().use_level)
Print.prototype.use_level = "5 -5"
console.log(a.use_level)
console.log(a.use_level)
console.log(Print()._mutable_options)
//Print().sp(a).log()
//Print.prototype.use_level = 4
*/
