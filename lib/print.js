...
Print.prototype = {
  ...
  _mutable_options: function() {
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     modify_string.max_line_characters = this.line_denote
     ...
     if ( msg instanceof Print ) {
       ...
       return modify_string.boolean(msg.toString())
     }
     else if ( msg !== msg ) { // NaN is the only js object which is not equal to itself
       return modify_string.not_a_number("NaN")
     }
     else if ( msg instanceof Error ) {
       var stack_string = msg.stack.split("\n")
       modify_string.namespace(stack_string.splice(0,1) + this.denote_line)
       return modify_string.function_body(stack_string.join(this.denote_line))
     }
     else if ( typeof msg === "undefined" ) {
       return modify_string.undefined("undefined")
     }
       ...
       for ( var n = 0; n < level; n++ ) {
         for ( var x = 0; x < this._cache[n].length; x++ ) {
           if ( typeof msg === "object" && msg === this._cache[n][x] ) {
             ...
             modify_string.scope_container(" } ]]", this.denote_line+compound, this.max_line_characters)
             return {formated: modify_string.formated, plain: modify_string.plain}
           }
         }
       }
       ...
       if ( typeof msg === "function" ) {
         ...
         if ( !value_keys_length ) {
           if ( proto_keys_length ) {
             ...
             formated += next_level.formated
             plain += next_level.plain
           } else {
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( !is_array ) {
           ...
           console.log(111111, plain)
           var more = this._stringify_object(o)
           formated += more.formated
           plain += more.plain
           console.log(222222, plain + "---" + object_separator)
           modify_string.colon(":" + object_separator)
           formated += modify_string.formated
           plain += modify_string.plain
           console.log(333333, plain)
         }
...
console.log(Print().set_option({max_depth: 3, max_line_characters: 4, theme: "light"}).sp(a).log())//.toString())//.set_option({theme: "light"}).log(a)
