  ...
  get pass() {
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     q}
       ...
       compress_sep = this.denote_line.toString()
       object_separator = this.denote_space+this.denote_space.toString()
     }
     ...
     if ( msg && (msg instanceof Print.prototype.Shim) ) {
       ...
       for ( var n = 0; n < level; n++ ) {
         for ( var x = 0; x < this._cache[n].length; x++ ) {
           if ( typeof msg === "object" && msg === this._cache[n][x] ) {
             ...
             var circ_text = modify_string.green("-- Circular duplicate of { "+ Object.keys(this._cache[n][x]).toString())
             if ( circ_text.length > 40 )
                circ_text = circ_text.substr(0, 25) + "..."
              circ_text += "} --"
             return circ_text
           }
         }
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( !is_array ) {
           ...
           // printed onlu via the _stringify_object below.
...
var a = [1,2,3,4]
var up = Print().set_option({use_title: false, use_color: false})

var o = {b: "bbb"}
o["ccccc"] = o

up.log({cool: "joe", a: o, b: {c: 22, d: 555, ff: {rr:22313, ggg:432}}})
//up.log(a)
