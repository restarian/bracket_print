...
Print.prototype = Object.create(
  {
    level: 1,//typeof this.level !== "undefined" ? this.level : 1,
    use_color: true,//typeof this.use_color !== "undefined" ? !!this.use_color : true,
    use_title_stamp: true,//typeof this.use_title_stamp !== "undefined" ? !!this.use_title_stamp: true,
    log_title: "",//typeof this.log_title !== "undefined" ? this.log_title : "",
    theme: "light",//typeof this.theme !== "undefined" ? this.theme : "light",
    compress_level: 2,//typeof this.compress_level !== "undefined" ? this.compress_level : 2,
    use_title: true,//typeof this.use_title !== "undefined" ? !!this.use_title : true,
    ...
    depth_limit: 130,//typeof this.depth_limit !== "undefined" ? (parseInt(this.depth_limit) || 130) : 130,
    character_limit: Math.pow(2,28),//typeof this.character_limit !== "undefined" ? this.character_limit : Math.pow(2,28),
    compress_function: false,//typeof this.compress_function !== "undefined" ? !!this.compress_function : false,
    denote_quoting: "\"",//typeof this.denote_quoting !== "undefined" ? this.denote_quoting : "\"",
    denote_line: "\n",//typeof this.denote_line !== "undefined" ? this.denote_line : "\n",
    denote_tab: "\t",//typeof this.denote_tab !== "undefined" ? this.denote_tab : "\t",
    denote_space: " ",//typeof this.denote_space !== "undefined" ? this.denote_space : " ",
...
Print.prototype.set_option = function() {
 ...
 p._is_chained = true
  ...
  //    arguments[i] = arguments[i]._mutable_options

    for ( var n in arguments[i] ) {
      if ( n in p ) {
        p._internal_set(n, arguments[i][n])
      } else {
        p.new_copy({level: Print.prototype.internal_level}).sp("Option", n, "is not a brackit print option. Use ._mutable_options to see all user configurable options:")
...
// to control the printing of messages from within this library.
Print.prototype.internal_level = 2,
Print.prototype._set_level = [-Infinity, Infinity],
Print.prototype.__defineSetter__("set_level", function(value) {
  ...
  value.toString().replace(/\ +/g, "").split(",").forEach(function(val) {
    ...
    if ( parseInt(range[0]) !== parseInt(range[0]) )
      return Print().log("The value used in set_level can not be parsed as an integer:", range[0])
    parsed.push(parseInt(range[0]))
    if ( range.length < 2 )
      range[1] = range[0]
    else if ( parseInt(range[1]) !== parseInt(range[1]) )
      Print().log("The value used in set_level can not be parsed as an integer:", range[1])
...
Print.prototype.__defineGetter__("clear", function() {
 ...
 Print.prototype._print_command = function(call_name) {
   ...
   var p = this._is_chained && this || Print(this)
   p._is_chained = true
   p._last_command = call_name.replace(/get\ /, "")
   return (function() {
 ...
 Print.prototype.__defineGetter__("_chain", function() {
   ...
   return function() {
     ...
     for ( var i = 0; i < arguments.length; i++ ) {
       ...
       if ( this._colors.plain.length && typeof this["denote_"+this._last_command] !== "undefined" ) {
         // Add the last separator relating to the previous printing command called
         this._colors.plain += this["denote_"+this._last_command]
         this._colors.formated += this["denote_"+this._last_command]
       }

       // The buffer will be clear here for each argument passed in to the print command.
       this._serialize(arguments[i], indentation)
       this._colors.formated += this._colors.closing_tag||""
       this._cache = []
     }
     return this
   }
 ...
 Print.prototype.__defineGetter__("log", function() {
   ...
   if ( !in_range )
     ...
     if ( this._colors.formated ) {
       ...
       var c_args = [(color_copy.formated||color_copy.plain)+this._colors.formated]
 ...
 Print.prototype._serialize = function(msg, indent_string, previous_indent, level) { // previous_indent will be placed at the start of each new object in the output (used internally)
   ...
   store_text.theme = this.theme + "_level_" + this.level
   store_text.character_limit = this.character_limit
   ...
   if ( store_text.plain.length >= this.character_limit )
    return
   ...
   if ( msg instanceof Print ) {
     ...
     store_text.namespace(stack_string.splice(0,1) + line_separator)
     store_text.function_body(stack_string.join(line_separator))
   }
     ...
     if ( typeof msg === "function" ) {
       ...
       if ( !this.compress_function ) {
        ...
        // This needs to be what the actual newline is for the platform because function text must maintain newlines to function properly.
         var line_denote = this.denote_line
         // Replace every newline in the function string with current indentation string the text and then the desired newline (this.denote_line).
         ...
         f_body.replace(/(.*)(\n+)/g, function() {
           ...
           if ( has_ascii )
            store_text.indent(body_indent)

           has_end_new_line = false
           var indent = line_denote
           if ( compress_level < 4 ) {
           ...
           store_text.function_body(arguments[1]||"")
           store_text.indent(indent)
         })
         if ( !has_end_new_line )
          store_text.remove_call(-1)
       } else {
