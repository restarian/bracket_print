   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg instanceof Print ) {
       ...
       if ( typeof msg === "function" ) {
         ...
         f_parameter.split(",").forEach(function(val) {
         ...
         if ( !this.compress_function ) {
           //var colors = new store_text.constructor()
           var line = this.denote_line
           f_body = f_body.replace(/\n/g, function(match, index, base) {
              return line + compound + indent_string
           })
           store_text.function_body(f_body)
         } else {
           store_text.function_body("..."+compress_separator)
         }
         store_text.scope_container(this.denote_line + "}")
         store_text.comma(",")
       }
       else {
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( cnt === 1 || is_literal_object || this.compress_level === 1 ) {
           store_text.indent(line_separator + compound + indent_string)
           if ( cnt === 1 && msg.valueOf() !== msg ) {
             store_text.namespace("[[PrimitiveValue]]")
             store_text.colon(":" + compress_separator)
             this._stringify_object(msg.valueOf(), compound + indent_string, level+1)
         ...
         if ( level < this.max_depth-1 || !is_literal_object ) {
            ...
            store_text.namespace("["+(is_array && "Array" || "Object") + " with length of "+(value_keys_length+proto_keys_length)+"]")//, indent_string, compound + indent_string)
         }
         ...
         if ( cnt === value_keys_length || cnt === value_keys_length+proto_keys_length ) {
           ...
           store_text.remove_call(-1)
           store_text.indent(line_separator + compound)
...
*/
//Print().set_option({max_depth: 5, compress_level: 2, compress_function: !true, theme: "light", indentation_string: "+"}).log({aaaa: {bb: {rr: a}}})//, String)
