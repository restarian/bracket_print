...
Print.prototype.Shim.prototype = {
  ...
  mutable_options: function() {
    ...
    return {
      ...
      platform: typeof this.platform !== "undefined" ? this.platform : typeof module === "object" && "nodejs" || "browser",
       ...
       if ( typeof msg === "function" ) {
         ...
         if ( this.compress_function ) {
           ...
           if ( msg.match(/\[native code\]/i) )
              ...
              f_text = this.denote_line + modify_string.darkGrey(compound) + modify_string.lightGrey(msg.toString().replace(/(.*)\}[\ ,\n]*/g, "$1"))
                .replace(/\n/g, modify_string.darkGrey(this.denote_line+ compound)) + modify_string.darkGrey(this.denote_line + compound) + modify_string.lightGrey("}")
         }
         return f_text
       }
       else {
         ...
         if ( !value_keys_length ) {
           if ( proto_keys_length ) {
              b_text += modify_string.cyan(is_array&&"]"||"}") + modify_string.lightGrey(".__proto__ = ") + modify_string.cyan("{") + compress_sep +
              ...
              b_text += modify_string.cyan(is_array&&"]"||"}") + modify_string.lightGrey(",") + object_separator
           }
         }
         m += b_text
       }
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( cnt === 1 || is_literal_object || this.compress_level === 1 ) {
           ...
           if ( cnt === 1 && msg.valueOf() !== msg ) {
             ...
             more += modify_string.lightGrey(",") + compress_sep + modify_string.darkGrey(indent_string)
...
//         character_count += (value+","+object_separator).length
         ...
         if ( cnt === value_keys_length || cnt === value_keys_length+proto_keys_length ) {
           ...
           str_add = str_add.substr(0, str_add.length-modify_string.lightGrey(",").length-object_separator.length) + compress_sep + modify_string.darkGrey(compound)
          ...
          //    str_add += indent_string + modify_string.red("__proto__") + modify_string.blue(":") + modify_string.cyan("{") + compress_sep + compound
              ...
              str_add += modify_string.cyan(is_array&&"]"||"}") + modify_string.lightGrey(",") + object_separator
         }
       ...
       m += str_add
       m = m.substr(0, m.length-modify_string.lightGrey(",").length-object_separator.length)
       if ( modify_string instanceof Print.Browser_colors ) {
         modify_string.css_arguments.splice(-2)
       }
       if ( !level )
...
Print.Terminal_colors.prototype = {
   ...
   format: function(args) {
     ...
     return this.theme_transform(args.callee.name||args[1], this.theme)+args[0].toString()
   },
 ...
 Print.Html_colors.prototype = {
   ...
   darkGrey: function darkGrey() { return this.format(arguments) },
   lightGrey: function lightGrey() { return this.format(arguments) },
 ...
 Print.Browser_colors.prototype = {
   ...
   format: function(args) {
     ...
     this._css_arguments[1] = "color: "+args.callee.name||args[1]//this.theme_transform(args.callee.name, this.theme))
     //     t"%c"+args[0].toString()his._css_arguments[1] = "color: "+this.default_color)//this.theme_transform(this.default_color, this.theme))
...
;(function() {
  ...
  // printing data and any new data passed into print_obj will also be stored so that is print_obj.sp("more") is called, print_obj.toString() will be "some data more".
  // This is why the Print.Shim prototype is copied into the Print prototype. All of the commands used in the Print call chain can be stared at a root level which can be
  // used to create new storage Objects.
  var main_sequence = {}
