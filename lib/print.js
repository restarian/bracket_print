...
var Print = function(setting) {
  ...
  if ( Object.prototype.toString.call(setting) === "[object Arguments]" )
    setting = setting[0]
  ...
  else if ( setting instanceof Print ) {
...
//          p.set_option({level: "err"}).log("Option", n, "is not a brackit print option. Use _mutable_options() to see all user configurable options.")
          p.new_copy().sp("Option", n, "is not a brackit print option. Use _mutable_options to see all user configurable options:")
  ...
  //    set_level: typeof this.set_level !== "undefined" ? this.set_level: Infinity,
      level: typeof this.level !== "undefined" ? this.level : 1,
  ...
  get clear() {
   ...
   _serialize: function(msg, indent_string, previous_indent, level) { // previous_indent will be placed at the start of each new object in the output (used internally)
     ...
     if ( store_text.plain.length >= this.max_characters )
      return
...
Print._mapping = function(str, command_name) {
  ...
  if ( this.plain.length === this.max_characters )
    return this
  ...
  if ( this.plain.length + plain.length > this.max_characters - truncated_msg.length )
    plain = plain.substr(0, this.max_characters - this.plain.length - truncated_msg.length ) + truncated_msg
