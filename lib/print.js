...
//          p.set_option({debug_level: "err"}).log("Option", n, "is not a brackit print option. Use _mutable_options() to see all user configurable options.")
  ...
  get _mutable_options() {
    ...
    return {
      ...
      approximate_max_characters: typeof this.approximate_max_characters === "number" ? this.approximate_max_characters : 10000,
      ...
      character_limit: typeof this.character_limit === "number" ? this.character_limit || 2 * 1000 * 1000 : 2 * 1000 * 1000,
   ...
   get log() {
     ...
     return (function() {
       ...
       if ( this._output ) {
         ...
         var title_str = this._colors.title(this.use_title && (this.log_title || this.use_title_stamp) ? "["+(this.use_title_stamp
   ...
   _stringify_object: function(msg, indent_string, previous_indent, level) { // previous_indent will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg instanceof Print ) {
       ...
       if ( value_keys_length && typeof msg === "object" && msg.valueOf() === msg )
       ...
       if ( typeof msg === "function" ) {
         ...
         if ( !this.compress_function ) {
          ...
          var color_copy
          if ( this.platform.toLowerCase() === "terminal" ) {
            color_copy = new Print.Terminal_colors()
          }
          else if ( this.platform.toLowerCase() === "browser" ) {
            color_copy = new Print.Browser_colors()
          }
       ...
       for ( var x in keys ) { // Loop through the Object values
         ...
         // Objects like browser self, window and document.
           if ( level < this.max_depth-1 || !is_literal_object ) {
              ...
              store_text.namespace("["+(is_array && "Array" || "Object") + " with "+(keys.length)+" properties]")//, indent_string, previous_indent + indent_string)
           }
...
// TODO: format should not add color to non-ansi characters strings
