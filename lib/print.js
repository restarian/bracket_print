...
Print.Addon_proto.prototype = {
  ...
  set_option: function() {
    ...
    var option = this._mutable_options()
    for ( var i = 0; i < arguments.length; i++ )
      for ( var n in arguments[i] )
        if ( n in option ) {
          this[n] = arguments[i][n]
        } else {
          this.always().log("Option", n, "is not a brackit print option. Use _mutable_options() to see all user configurable options.")
        }
    return this
  },
  new_copy: function() {
    ...
    for ( var x = 0; x < arguments.length; x++ )
      if ( arguments[x] instanceof this ) {
      //if ( arguments[x] instanceof this.constructer ) {
    ...
    arguments.length++
    return this.constructer(arguments)
  },
  _mutable_options: function() {
    ...
    return {
      ...
      debug: typeof this.log_level !== "undefined" ? this.log_level : "verbose",
  ...
  get pass() {
   ...
   print_command: function() {
     ...
     if ( this.debug === "verbose" || this._always || this.log_level <= parseInt(this.debug) )
       return (function() {
   ...
   get _chain() {
     ...
     return (function() {
       ...
       for ( var i = 0; i < arguments.length; i++ ) {
         ...
         this._colors.use_color = this.use_color
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg instanceof this.constructor ) {
       ...
       for ( var n = 0; n < level; n++ ) {
         for ( var x = 0; x < this._cache[n].length; x++ ) {
           if ( typeof msg === "object" && msg === this._cache[n][x] ) {
             ...
             return modify_string.yellow(circ_text) + modify_string.lightGrey("{ "+obj_qualifier+" }") + modify_string.yellow(" ]]")
           }
         }
       }
       ...
       if ( typeof msg === "function" ) {
         var f_text = ""
         if ( msg.toString().match(/\[native code\]/i) ) {
            f_text = modify_string.blue(msg.toString().replace(/(.*)\n*/g, "$1"))//+compound
         }
         else if ( this.compress_function ) {
           f_text = msg.toString().match(/(function[^\{]+).*/i).pop()
           f_text = modify_string.black(f_text)+modify_string.cyan("{")+modify_string.black("...")+modify_string.cyan("}")
         }
         else {
            var line = this.denote_line
            f_text = modify_string.black(msg.toString().replace(/(.*)\}[\ ,\n]*/g, "$1"))
            .replace(/\)\ *\{/, ") "+modify_string.cyan("{")+modify_string.darkGrey(null))
            .replace(/\n/g, function(match, index, base) { return modify_string.darkGrey(line + compound) })
                + modify_string.lightGrey(this.denote_line + compound) + modify_string.cyan("}")
         }
         m += f_text
       }
       else {
         ...
         if ( !value_keys_length ) {
           if ( proto_keys_length ) {
              b_text += modify_string.cyan(is_array&&"]"||"}") + modify_string.red(".__proto__ = ") + modify_string.cyan("{") + compress_sep +
                  modify_string.darkGrey(compound + indent_string)
              ...
              b_text += modify_string.cyan(is_array&&"]"||"}") + modify_string.darkGrey(",") + object_separator
           }
         }
         m += b_text
       }
       ...
       for ( var o in msg ) { // Loop through the Object values
         ...
         if ( cnt === 1 || is_literal_object || this.compress_level === 1 ) {
           str_add += compress_sep + modify_string.darkGrey(compound + indent_string)
           if ( cnt === 1 && msg.valueOf() !== msg ) {
             var more = modify_string.red("[[PrimitiveValue]]") + modify_string.black(":") + object_separator
             more += this._stringify_object(msg.valueOf(), indent_string. compound + indent_string, level+1)
             more += modify_string.darkGrey(",") + compress_sep + modify_string.darkGrey(indent_string)
         ...
         if ( !is_array ) {
           ...
           var more = this._stringify_object(o) + modify_string.black(":") + object_separator
           character_count += more.length
           str_add += more
         }
         ...
         str_add += value + modify_string.lightGrey(",") + object_separator
...
//         character_count += (value+","+object_separator).length
         ...
         if ( cnt === value_keys_length || cnt === value_keys_length+proto_keys_length ) {
           ...
           str_add = str_add.substr(0, str_add.length-modify_string.darkGrey(",").length-object_separator.length)
           if ( modify_string.use_color && modify_string instanceof Print.Browser_colors ) {
              modify_string.css_arguments.splice(-2)
           }
           str_add += compress_sep + modify_string.darkGrey(compound)
           ...
           if ( value_keys_length && proto_keys_length && cnt === value_keys_length ) {
              str_add += modify_string.cyan(is_array&&"]"||"}") + modify_string.red("__proto__") + modify_string.black(":") +
                  object_separator + modify_string.cyan("{") + compress_sep + compress_sep
          ...
          //    str_add += indent_string + modify_string.red("__proto__") + modify_string.blue(":") + modify_string.cyan("{") + compress_sep + compound
              ...
              str_add += modify_string.cyan(is_array&&"]"||"}") + modify_string.darkGrey(",") + object_separator
           }
         }
       ...
       m += str_add
       m = m.substr(0, m.length-modify_string.darkGrey(",").length-object_separator.length)
       if ( modify_string.use_color && modify_string instanceof Print.Browser_colors ) {
         modify_string.css_arguments.splice(-2)
       }
       if ( !level )
       ...
       if ( this._is_from_object && this.quoting.toLowerCase() === "double") {
         return modify_string.darkGrey("\"")+modify_string.magenta(msg)+modify_string.darkGrey("\"")
       }
   ...
   _descend: function(command_name, args) {
     return this[command_name].apply(this, args)
   },

}

Print.prototype = new Print.Addon_proto()

;(function() {
  // Data is stored in the print Object when a Print sub-Object is assigned to a variable, E.g. var print_obj = Print().sp("some data"), print_obj will hold the
  // printing data and any new data passed into print_obj will also be stored so that if print_obj.sp("more") is called, print_obj.toString() will be "some data more".
  // This is why the Print.Shim prototype is copied into the Print prototype. All of the commands used in the Print call chain can be started at a root level which is
  // used to create independant print Objects.
  var p = Print.prototype.__proto__

  for ( var n in p ) {
    if ( n.charAt(0) !== "_" ) {
      var f = function() {
        // arguments.callee.name
        var This = new Print()//this.constructor()
        return This._descend(arguments.callee.command_name, arguments)
      }
      f.command_name = n
      Print.prototype[n] = f
    }
...
Print.Terminal_colors.prototype = {

   format: function(args) {

     if ( !this.use_color || ( args[0] !== null && ( !args[0] || !args[0].toString().replace(/\ */, "").length ) ) )
        return (args[0]||"").toString()

      if ( args[0] == null )
        args[0] = ""

     return this.theme_transform(args[1]||args.callee.name, this.theme)+args[0].toString()
   },
   ...
   white: function() { arguments[1] = "\033[0;97m"; return this.format(arguments) },
   lightGrey: function() { arguments[1] = "\033[0;37m"; return this.format(arguments) },
   darkGrey: function() { arguments[1] = "\033[0;90m"; return this.format(arguments) },
   cyan: function() { arguments[1] = "\033[0;36m"; return this.format(arguments) },
   magenta: function() { arguments[1] = "\033[0;35m"; return this.format(arguments) },
   red: function() { arguments[1] = "\033[0;31m"; return this.format(arguments) },
   blue: function() { arguments[1] = "\033[0;34m"; return this.format(arguments) },
   yellow: function() { arguments[1] = "\033[0;33m"; return this.format(arguments) },
   green: function() { arguments[1] = "\033[0;32m"; return this.format(arguments) },
   black: function() { arguments[1] = "\033[0;30m"; return this.format(arguments) },
 }
 ...
 Print.Html_colors.prototype = {
   ...
   format: function(args) {
     ...
     return "<span style='color:"+this.theme_transform(args[1]||args.callee.name, this.theme)+";'>"+args[0].toString()+"</span>"
   },
   theme_transform: function(c, announce) {
   ...
   darkGrey: function darkGrey() { arguments[1] = "#323232"; return this.format(arguments) },
   lightGrey: function lightGrey() { arguments[1] = "#939393"; return this.format(arguments) },
   cyan: function cyan() { arguments[1] = "#249e93"; return this.format(arguments) },
   ...
   yellow: function yellow() { arguments[1] = "#f4d400"; return this.format(arguments) },
 ...
 Print.Browser_colors.prototype = {
   ...
   format: function(args) {
     ...
     this._css_arguments.push("color: "+this.theme_transform(args[1]||args.callee.name, this.theme))
   ...
   black: function black() { return this.format(arguments) },
          ...
          // The call commands on the Shim prototype are only used when debug is set to verbose.
    //      if ( this.debug === "verbose" || this._always ) {
  //k          return new this.Shim(this._mutable_options())[cmd].apply(this, arguments)
