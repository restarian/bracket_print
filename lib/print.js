  ...
  _mutable_options: function() {
   ...
   _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
     ...
     if ( msg instanceof Print ) {
       ...
       return modify_string.number(msg.toString())
     }
     else if ( typeof msg === "string" ) {
       // this is set if the sting is from an object
       if ( this._is_from_object && this.quoting.toLowerCase() === "double") {
         modify_string.double_quote("\"")
         modify_string.string(msg)
         return modify_string.single_quote("\"")
       }
       else if ( this._is_from_object && this.quoting.toLowerCase() === "single") {
         modify_string.single_quote("\'")
         modify_string.string(msg)
         return modify_string.single_quote("\'")
         }
       else {
         return modify_string.string(msg)
       }
     }
     else {
       return modify_string.namespace(msg.toString())
     }
   },
}
...
// This is the first level prototype Object for any Color convering plugin.
  ...
  this[val] = function(str, denote_line, max_line_characters, flag) {
    ...
    var plain = formated = ""
    plain += this._add_line_break(str.toString(), denote_line, max_line_characters)
    if ( !flag )
      this._plain += plain
    ...
    formated = this.format(plain, this.__proto__[this.theme][arguments.callee.command_name], denote_line, max_line_characters)
    // Call the format function which are specified in the first level prototype of the constructor
    if ( !flag )
      this._formated += formated

    return {plain: plain, formated: formated}
  }
...
Print().set_option({max_depth: 3, max_line_characters: 30, theme: "light"}).log({})//.set_option({theme: "light"}).log(a)
