...
var Wizkit_print = function() {
  // This constructor accepts three different types of parameters: a string to use as a logging title, a Wizkit_print instance
  // to copy settings from, or a arguments object containing either of the previous two types. This is done so
  // that the new_copy member can easily pass its' settings to this contstructor for copying.

  // This fumction will always act as a Constructer.
  if ( !(this instanceof arguments.callee) )
    return new arguments.callee(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5])
  ...
  // Settings will be null if no Wizkit_print was passed in to the constructer so the defaults are set.
  if ( !settings )
    settings = this.mutable_options()

  // Manually set the settings to this instance.
...
Wizkit_print.prototype = {
  ...
  set_option: function() {
    // The options can be set with this member which returns itself or the poperties. Any property which does not
    // contain a _ a the beginning is user configurable.
    return new this.Wizkit_print_logger(this.mutable_options()).set_option.apply(this, arguments)
  },
  new_copy: function() {
    // new_copy sinply calls the constructor with a Wizkit_print instance so this just forwards the arguments as well.
    return this.Wizkit_print_logger.prototype.new_copy.apply(this, arguments)
  },
  mutable_options: function() {
    ...
    return this.Wizkit_print_logger.prototype.mutable_options.call(this)
  },
  Wizkit_print_logger: function(settings) { // The Wizkit_print constructer is sub-classed here in Wizkit

    // This is the main constructor for the Wizit_print library. Any prototype members for this function will be copied
    // to the Wizkit_print function as well. It is this way that an instance can create anther instance. Any prototype Objects
    // which are already assigned to the Wizkit_print constructor will not be overwriten.
    ...
    // These settings are used internally, (hence the _ at the begining).
    ...
    // The platform property holds which method of coloring to used based on the platorm it is ran on. The nodejs platform can
    ...
    else if ( this.platform.toLowerCase() === "console_html" || this.platform.toLowerCase() === "html" ) {
      this._colors = new Wizkit_print.Html_colors()
    }
  },
}

Wizkit_print.prototype.Wizkit_print_logger.prototype = {

  toColorString: function() {
    // Stores the current color formated text of the current print instance based on the platform
    return this._output
  },
  toString: function() {
    // Stores the current plain text of the print instance.
  ...
  new_copy: function() {
    ...
    for ( var x = 0; x < arguments.length; x++ )
      if ( arguments[x] instanceof Wizkit_print.prototype.Wizkit_print_logger ) {
  ...
  mutable_options: function() {
    ...
    return {
      ...
      denote_denote_tab: typeof this.denote_denote_tab !== "undefined" ? this.denote_denote_tab : "\t",
      denote_denote_space: typeof this.denote_denote_space !== "undefined" ? this.denote_denote_space : " ",
      denote_add: typeof this.denote_add !== "undefined" ? this.denote_add : ""

    }
  },
  get always() {
    // This ignores the current debug state for use with string managment in other scripts.
    var This = this
    return function() {
      this._always = true
      return This
  ...
  get _chain() {
    ...
    return function() {
      ...
      for ( var i = 0; i < arguments.length; i++ ) {
        ...
        if ( !!This._last_command && !This._ignore_separater && This[This._last_command] ) {
          This._output += This[This._last_command]
          This._colorless_output += This[This._last_command]
        }
...
//        var title_str = This.log_title ? this._colors.blue("[")+This.log_title+this._colors.blue("] ") : ""
  ...
  _stringify_object: function(msg, indent_string, compound, level) { // compound will be placed at the start of each new object in the output (used internally)
    ...
    if ( msg && (msg instanceof Wizkit_print.prototype.Wizkit_print_logger) ) {
      ...
      msg = modify_string.red(stack_string.splice(0,1)) + this.denote_line + modify_string.yellow(stack_string.join(denote_line))
    }
...
var fake_logger = new (function() { // The FakeWizkit_print is used to disable logging when debug is set to false (it is efficiant this way).
    ...
    for ( var o in Wizkit_print.prototype.Wizkit_print_logger.prototype )
      this[o] = f
...
// This duplicates the library into the Wizkit_print contstructor. It is this way that text can be stored in every call a member. The members
// return a new copy of the library which is created vhere.
for ( var n in Wizkit_print.prototype.Wizkit_print_logger.prototype ) {
    // Any call command which is not internally used (no _ at the begining), will have a redundant function returning itself to placate the
    // call command. It is important to keep the always functionality in order to toggle this function call.
    if ( typeof Wizkit_print.prototype[n] === "undefined" && n.charAt(0) !== "_" ) {
    var forward = function() {
      return (this.debug === "verbose" || this._always || arguments.callee.command_name === "always" ) && new this.Wizkit_print_logger(this.mutable_options())[arguments.callee.command_name].apply(this, arguments) || fake_logger
    }
