...
define(["require"], function(require) {
	...
	return function(msg, indent_string, previous_indent, level) {
	  ...
	  // (calling _serializer inside _serializer). this member can not be called outside this script with out breaking the the overlying
	  ...
	  if ( this.compress_level > 3 ) {
		 ...
		 // The lowest compression settings are the same as the sencond most above (two), except some ligic in the Object serializerr below.
	  ...
	  if ( msg instanceof (this.parent || function(){}) ) {
		...
		this._serializerr(msg.toString())
	  }
	  else if ( msg === null ) {
		...
		this._serializer(msg.toString())
		 ...
		 for ( var x = 0; x < keys.length; x++ ) {
			...
			// is reached in an Object which contains more Object to serializer. The Object serialization will be replaced with a String decribing the Object.
			...
			if ( cnt === keys.length ) {
			  ...
			  if ( msg.__proto__ && typeof msg.__proto__ === "object" && Object.keys(msg.__proto__).length ) {
				 ...
				 this.append_string("namespace", "__proto__")
				 this.append_string("colon", ":" + compress_separator)
				 this._serializerr(msg.__proto__, indent_string, previous_indent + indent_string, level+1)
			  }
			  this.append_string("indent", line_separator + previous_indent)
			  this.append_string("scope_container", is_array&&"]"||"}")
			  this.append_string("comma", "," + compress_separator)
