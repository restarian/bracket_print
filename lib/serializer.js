			...
			if ( typeof msg === "function" ) {
				...
				if ( this.truncate_function ) {
					...
					f_body.replace(/(^.*)([\n,\r]+)/mg, (function() {
						...
						var newline = (arguments[2].match(/[\n,\r]/g) || []).reduce(function(accumulator, value, index, arr) {
							if ( arr.length-1 !== index ) {
								has_end_new_line = true
								return accumulator + denote_line
							}
							return accumulator
						}, denote_line)

						var min_t = min_tab, min_s = min_space
						if ( this.compress_level >= 0 && initial_empty_line )
							this.append_string("function_body", arguments[1].replace(/^[\t,\n,\r,\s]+/, ""))
						else if ( this.indent_function ) {
							this.append_string("function_body", arguments[1].replace(/\t/g, function(match) {
								if ( --min_t > 0 ) 
									return "" 
								return match
