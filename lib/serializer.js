...
define(function(require) {
	...
	return function(msg, indent_string, previous_indent, level, is_qualifier) {
		...
		this.append_string("nan", "NaN")
			...
			var is_array = msg instanceof Array, is_function = false
			if ( typeof msg === "function" ) {
				is_function = true

				var f_str = msg.toString()
				...
				if ( msg.hasOwnProperty("prototype") ) 
					f_str = msg.toString().match(/function\s*([^\(]*)\(([^\)]*)\)\s*\{((?:.|[\n,\r\n])*)\}/im)
				else
					f_str = ["","","..arrow functions not supported in this version"]
					//f_str = msg.toString().match(/\s*(\()\(.*\)\s*  \{((?:.|[\n,\r\n])*)\}/im)

				var f_name = (f_str[1] || "").replace(/\s+$/, "")
				...
				this.remove_call(-1)
				this.append_string("parenthesis", (param[0] && this.compression < 3 && compress_separator || "") + ")" + (this.compression < 5 && denote_space || "") )
			...
			if ( !keys.length ) {
				...
				this.append_string("indent", ((!is_function && this.compression < 3) && line_separator || "") + previous_indent)
