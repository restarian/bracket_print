...
define(function(require) {
	...
	return function(msg, indent_string, previous_indent, level) {
		...
		// denote_line is used so that the function body can have simple newline replacements.
		...
		indent_string = typeof indent_string === "string" && indent_string || 
								(this.style_map[this.platform].denote_space + this.style_map[this.platform].denote_space)
		...
		// No indents and lines are spaces as well.
			indent_string = ""
			compress_separator = this.style_map[this.platform].denote_space
			line_separator = this.style_map[this.platform].denote_space
		}
		else if ( this.compress_level === 2 ) {
	  
			compress_separator = this.style_map[this.platform].denote_space
			line_separator = this.style_map[this.platform].denote_line
		}
		else {
		...
		if ( msg instanceof this.parent ) {
			...
			var stack_string = msg.stack.split(/[\n,\r]/).slice(1).map(function(val) { 
				return val.replace(/^\s*/, this.compress_level < 4 && (previous_indent + indent_string) || " " ) 
			}, this)
			...
			if ( typeof msg === "function" ) {
				...
				// if parameters are used in the function. Note: the compress_separator adjusts pending the this.compress_level.
				this.append_string("parenthesis", "(" + (param[0] && compress_separator || ""))
				...
				this.remove_call(-1)
				this.append_string("parenthesis", (param[0] && compress_separator || "") + ")" + compress_separator)
				this.append_string("bracket", "{" + compress_separator)
				...
				if ( !this.truncate_function ) {
					...
					if ( ! /[\n,\r]/.test(f_body) )
						f_body = denote_line + f_body + denote_line
					...
					// then indented where the current object parsing level is (if indent_function option is used).

					var min_tab = [], min_space = [] 
					if ( this.indent_function ) {
						(f_body.match(/^\s+/g)||[]).forEach(function(match) {
							min_tab = Math.min((match.match(/\t/g)||[]).length, min_tab) 
							min_space = Math.min((match.match(/\ /g)||[]).length, min_space)
						})

						min_tab = RegExp(Array.apply(null, {length:min_tab}).reduce(function(accumulator) {
							return accumulator + "\\t"
						}, "\\t"))

						min_space = RegExp(Array.apply(null, {length:min_space}).reduce(function(accumulator) {
							return accumulator + " "
						}, " "))
					}
					...
					f_body.replace(/(.*)([\n,\r]+)/g, (function() {
						...
						// Test if the line contains a white-space character. 
						var has_ascii = /\S/.test(arguments[1])
						// Any empty lines are removed in high compression level mode.
						...
						if ( this.compress_level >= 0 && first_match )
							this.append_string("function_body", arguments[1].replace(/^[\t,\n,\r,\s]+/, "***"))
						else if ( this.indent_function )
							this.append_string("function_body", arguments[1].replace(/^[\t,\ ]+/g, function() {
								return 
							})
							
							min_tab, "").replace(min_space, ""))
				...
				this.append_string(is_array&&"brace"||"bracket", is_array&&"["||"{" + 
						(this.compress_level < 3 && this.style_map[this.platform].denote_space || ""))
			 }
			 ...
			 for ( var x = 0; x < keys.length; x++ ) {
				...
				if ( !is_array ) {
					...
					// strings as this
				   if ( ! this._serializer(o, undefined, undefined, level) )
						return false
					this._is_qualifier = false 
					// Object separator makes it more human readable by adding space. This changes with the compress_level.
				   this.append_string("colon", ":" + compress_separator) 				
				}
			...
			if ( this._cache.length ) {
				if ( !this._is_qualifier || this.quote_qualifier )
