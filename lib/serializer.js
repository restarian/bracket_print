	...
	return function(msg, indent_string, previous_indent, level) {
	  ...
	  // A simple way to stop Object parsing when the output becomes too large. The actual size of the internal string is maintained by the color addon.
		...
		if ( msg instanceof this.parent ) {
			...
			// There is no need to put primitive Objects into the cache sense they can never be circular dependencies to another one.
			...
			if ( typeof msg === "function" ) {
			 ...
			 for ( var x = 0; x < keys.length; x++ ) {
				...
				if ( !is_array ) {
				   ...
				   // is the only type of qualifiers ecma allows.
