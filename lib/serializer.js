	...
	return function(msg, indent_string, previous_indent, level, is_qualifier) {
		...
		if ( msg instanceof this.parent ) {
			...
			if ( typeof msg === "function" ) {
				...
				var param = f_parameter.replace(/\ *,\ */g, ",").split(",")
				...
				if ( this.truncate_function ) {
					...
					// All of the superfluous tabs and spaces which precede new lines are removed sense they do not affect the text look but do affect 
					// the processing below.
					f_body = f_body.replace(/^[\t, ]$/mg, "")
					...
					if ( !/^[\t, ]*[\n,\r]/.test(f_body) && this.compression < 4 )
